...
export default class DocLayerEditSketchDemo extends Component {
  ...
  initSketchEditor = async () => {
    let sketchEditor1 = await SketchEditor.createInstance(this.mapView);
  ...
  startSaveFeature = async () => {
    ...
    let geoTypeName = '点';
    // 多点
    if (geoType === GeometryType.GeoMultiPoint) {
      geoTypeName = '点';
      // 属性
      ...
      feature = await Feature.createInstanceByParam(
        attribute,
        geometry,
        pntInfo
      );
    ...
    } else if (geoType === GeometryType.GeoMultiLine) {
      geoTypeName = '线';
      // 属性
      let attribute = { Name_chn: '自定义线1' };
    ...
    } else if (geoType === GeometryType.GeoMultiPolygon) {
      geoTypeName = '区';
      // 属性
      let attribute = { Name: '自定义区1' };
    ...
    if (this.state.addFeature != null) {
      let featureAppendEdit = await FeatureEdit.createInstanceByVectorLayer(
  ...
  tapMapView = async res => {
    ...
    let resDot = await Dot.createInstance(res.x, res.y);

    let pointF = await this.mapView.mapPointToViewPoint(resDot);

    let pointX = await pointF.getX();
    let pointY = await pointF.getY();
    let pointF1 = await PointF.createInstance(pointX - 40, pointY - 30);
    let pointF2 = await PointF.createInstance(pointX - 40, pointY + 30);
    let pointF3 = await PointF.createInstance(pointX + 40, pointY - 30);
    let pointF4 = await PointF.createInstance(pointX + 40, pointY + 30);
    ...
    //创建查询范围
    let dot1X = await dot1.getX();
    let dot1Y = await dot1.getY();
    let dot4X = await dot4.getX();
    let dot4Y = await dot4.getY();
    let rect = await Rect.createInstance(dot1X, dot1Y, dot4X, dot4Y);

    // 查询。得到Feature
    let featureQuery = await FeatureQuery.createInstanceByVectorLayer(
      this.state.vectorLayer
    );
    ...
    await queryDef.setRect(rect, SpaQueryMode.ModeMBRIntersect);
  ...
  startDeleteFeature = async () => {
    if (this.state.deleteFeature !== null) {
      let featureDeleteEdit = await FeatureEdit.createInstanceByVectorLayer(
        this.state.vectorLayer
      );
  ...
  modifyAttr = async () => {
    ...
    if (this.state.modifyFeature !== null) {
      ...
      if (this.state.geoType === GeometryType.GeoMultiPoint) {
        ...
        this.state.geoType === GeometryType.GeoMultiLine
      ) {
        let attrArray = JSON.parse(attribute);
        ...
        this.state.geoType === GeometryType.GeoMultiPolygon
      ) {
        let attrArray = JSON.parse(attribute);
  ...
  startModifyAttr = async () => {
    ...
    if (a > 0) {
      let featureEdit = await FeatureEdit.createInstanceByVectorLayer(
        this.state.vectorLayer
      );
