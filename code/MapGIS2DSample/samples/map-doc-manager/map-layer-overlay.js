...
export default class MapLayerOverlay extends Component {
  ...
  // 确认添加服务图层
  insertLayer = async () => {
    if (this.state.serverLayerTypeName === '高德服务图层') {
      // 2、插入服务图层是google服务图层，先移除离线图层，再添加图层
      this.removeLayer('离线瓦片地图');

      let serverLayer = await ImageLayer.createInstance();
      let amapMapServer = await ImageLayer.createMapServer(
        'AMapMercatorSatelliteMap',
      );
      // 设置服务图层的数据源
      await serverLayer.setMapServer(amapMapServer);
      // 设置图层名称
      await serverLayer.setName('高德服务图层');
      await this.map.insert(2, serverLayer);
      await this.mapView.forceRefresh();
    } else {
      // 2、插入的是离线瓦片地图，先移除Google服务图层，再添加离线瓦片地图
      this.removeLayer('高德服务图层');

      let offServerLayer = await ImageLayer.createInstance();
      let offlineTileMapServer = await ImageLayer.createMapServer(
      ...
      <View style={styles.container}>
        ...
        >
          <View style={style.modalView}>
            ...
            >
              <RadioButton style={style.radioButton} value={'高德服务图层'}>
                <Text style={styles.text}>高德服务图层</Text>
              </RadioButton>
