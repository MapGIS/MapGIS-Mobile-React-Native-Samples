...
export default class MapLayerControl extends Component {
  ...
  showAllLayer = async visible => {
    ...
    if (this.map !== null) {
      ...
      for (let i = 0; i < layerCount; i++) {
        ...
        let layerState = await mapLayer.getState();
        let layer = {
          id: i,
          name: name,
  ...
  checkLayerDetail = async item => {
    ...
    let isVisible = state === LayerState.Visible ? true : false; // 可见性
    // let scaleRange = await mapLayer.getScaleRange(); // 图层可见比例尺区间
    // let minScale = await scaleRange.getMinScale(); // 最小显示比
    // let maxScale = await scaleRange.getMaxScale(); // 最大显示比
