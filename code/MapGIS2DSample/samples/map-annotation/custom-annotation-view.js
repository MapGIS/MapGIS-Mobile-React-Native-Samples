...
export default class CustomAnnotationView extends Component {
  ...
  addConstantAnnotation = async () => {
    let image = await Image.createInstanceByLocalPath(ANN_FILE_PATH);

    // 标注点位置
    let position = await Dot.createInstance(12724100, 3573729.6);
    // 标注名称
    let name = '黄鹤楼';
    // 标注内容
  ...
  addLabel = async (image, dot, name, content, img) => {
    ...
    // 初始化标注对象
    let annotation = await Annotation.createInstanceByUID(
      img,
      name,
      content,
      dot,
