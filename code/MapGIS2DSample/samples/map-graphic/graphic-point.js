...
import { MGMapView, Dot, GraphicMultiPoint } from '@mapgis/uniform-core-react-native';
...
export default class MapGraphicPoint extends Component {
  ...
  openMap = async () => {
    ...
    let dotArray = [];
    let dot1 = await Dot.createInstance(12697530, 3593327);
    let dot2 = await Dot.createInstance(12736224, 3570660);
    let dot3 = await Dot.createInstance(12766215, 3612566);
    dotArray.push(dot1);
    dotArray.push(dot2);
    dotArray.push(dot3);
    this.graphicMultiPoint = await GraphicMultiPoint.createInstance();
