...
export default class MapClipAnalysis extends Component {
    ...
    this.mapLoadListener = DeviceEventEmitter.addListener(
      ...
      async res => {
        if (res.DidFinishLoadingMap) {
          ...
          let dotObj = await Dot.createInstance(12721901.5, 3560170.1);
          let resolution =
   ...
   */
  async init() {
    this.spaAnalysisObj = await SpatialAnalysis.createInstance();

    //裁剪数据图层
    this.graphicsOverlayData = await this.mapView.getGraphicsOverlay();
    //裁剪叠加层
    this.graphicOverlayResObj = await GraphicsOverlay.createInstance();
    ...
    //裁剪多边形点坐标（地图坐标）
    let dotArr = [];
    let dotObj1 = await Dot.createInstance(12720512.5, 3558371.2);
    let dotObj2 = await Dot.createInstance(12720358.6, 3561554.1);
    let dotObj3 = await Dot.createInstance(12723017.1, 3561615.5);
    let dotObj4 = await Dot.createInstance(12723074.1, 3559360.3);
    let dotObj5 = await Dot.createInstance(12720512.5, 3558371.2);
    dotArr.push(dotObj1);
    dotArr.push(dotObj2);
    dotArr.push(dotObj3);
    dotArr.push(dotObj4);
    ...
    //创建并初始化裁剪多边形图形
    let graphicPolygonObj = await GraphicPolygon.createInstance();
  ...
  async initClipGeometry() {
    ...
    //初始化查询对象
    let featureQueryObj = await FeatureQuery.createInstanceByVectorLayer(
      vectorLayer
    );
    //设置查询条件
    ...
    await queryDef.setPagination(1, 1000);
    await featureQueryObj.setQueryDef(queryDef);
    //进行查询
  ...
  clipByCircle = async () => {
    ...
    if (this.geometry !== null && this.geoPolygon !== null) {
      //得到裁剪区
      let geoClip = await GeometryOperator.clipGeometryByGeometry(
        this.geometry,
        this.geoPolygon,
        ClipType.INCLIP,
        0,
