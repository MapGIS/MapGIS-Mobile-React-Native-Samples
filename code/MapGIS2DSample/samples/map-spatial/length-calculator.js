...
export default class MapCalculateLength extends Component {
  ...
  async calLength() {
    ...
    // let length = await geoVarLine.calLength();

    //第二种方法：计算线实地或者平面长度
    let map = await this.mapView.getMap();
    let refData = await map.getSRSInfo();
    let geoVarLineObj = new GeoVarLine();
    geoVarLineObj.ObjId = geoVarLine.ObjId;
    let length = await geoVarLineObj.calLengthOfSRef(refData);
