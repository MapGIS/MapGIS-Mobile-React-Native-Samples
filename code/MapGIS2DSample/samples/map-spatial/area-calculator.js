...
export default class MapCalculateArea extends Component {
  ...
  async calArea() {
    ...
    let graphicTextObj = await GraphicText.createInstance();
    //设置多边形点集与样式
    ...
    let geoPolygonObj = new GeoPolygon();
    geoPolygonObj.ObjId = geoPolygon.ObjId;
    let area = await geoPolygonObj.calAreaOfSRef(refData);
