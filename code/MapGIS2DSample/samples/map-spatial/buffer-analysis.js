...
export default class MapBufferAnalysis extends Component {
    ...
    this.mapLoadListener = DeviceEventEmitter.addListener(
      ...
      async res => {
        if (res.DidFinishLoadingMap) {
          ...
          let dotObj = await Dot.createInstance(12729985, 3606886);
          let resolution =
  ...
  pointBuffer = async () => {
    ...
    let geoPolygons = await GeometryOperator.buffer(
      geometry,
      2000,
      CapStyles.ROUND,
    );
  ...
  lineBuffer = async () => {
    ...
    //点坐标（地图坐标）
    let dotArr = [];
    let dotObj1 = await Dot.createInstance(12719530, 3603327);
    let dotObj2 = await Dot.createInstance(12736224, 3620660);
    dotArr.push(dotObj1);
    dotArr.push(dotObj2);
  ...
  areaBuffer = async () => {
    ...
    //根据屏幕的高宽自定义坐标点
    let pointFObj1 = await PointF.createInstance(
      (this.mapViewWidth * 1.0) / 10,
      (this.mapViewHeight * 1.0) / 10
    );
    let pointFObj2 = await PointF.createInstance(
      (this.mapViewWidth * 4.0) / 10,
      (this.mapViewHeight * 1.0) / 10
    );
    let pointFObj3 = await PointF.createInstance(
      (this.mapViewWidth * 5.5) / 10,
      (this.mapViewHeight * 2.8) / 10
    );
    let pointFObj4 = await PointF.createInstance(
      (this.mapViewWidth * 3.5) / 10,
      (this.mapViewHeight * 4.5) / 10
    );
    let pointFObj5 = await PointF.createInstance(
