...
export default class MapOverlayAnalysis extends Component {
   ...
   */
  async initView() {
    this.pointFObj1 = await PointF.createInstance(
      (this.mapViewWidth * 1.0) / 10,
      (this.mapViewHeight * 1.0) / 10
    );
    this.pointFObj2 = await PointF.createInstance(
      (this.mapViewWidth * 4.0) / 10,
      (this.mapViewHeight * 1.0) / 10
    );
    this.pointFObj3 = await PointF.createInstance(
      (this.mapViewWidth * 5.5) / 10,
      (this.mapViewHeight * 2.8) / 10
    );
    this.pointFObj4 = await PointF.createInstance(
      (this.mapViewWidth * 3.5) / 10,
      (this.mapViewHeight * 4.5) / 10
    );
    this.pointFObj5 = await PointF.createInstance(
      (this.mapViewWidth * 2.0) / 10,
      (this.mapViewHeight * 4.5) / 10
    );
    this.pointFObj6 = await PointF.createInstance(
      (this.mapViewWidth * 3.0) / 10,
      (this.mapViewHeight * 1.5) / 10
    );
    this.pointFObj7 = await PointF.createInstance(
      (this.mapViewWidth * 5.5) / 10,
      (this.mapViewHeight * 1.5) / 10
    );
    this.pointFObj8 = await PointF.createInstance(
      (this.mapViewWidth * 7.0) / 10,
      (this.mapViewHeight * 3.0) / 10
    );
    this.pointFObj9 = await PointF.createInstance(
      (this.mapViewWidth * 6.0) / 10,
      (this.mapViewHeight * 3.0) / 10
    );
    this.pointFObj10 = await PointF.createInstance(
    ...
    //叠加分析结果层
    this.graphicsOverlayResultObj = await GraphicsOverlay.createInstance();
  ...
  async drawPolygon() {
    ...
    dotArrA.push(point1);
    //创建目标图形
    let graphicPolygonAObj = await GraphicPolygon.createInstance();
    await graphicPolygonAObj.setPoints(dotArrA);
    ...
    dotArrB.push(point6);
    let graphicPolygonBObj = await GraphicPolygon.createInstance();
    await graphicPolygonBObj.setPoints(dotArrB);
  ...
  calIntersectionBtn = async () => {
    ...
    // 两个区几何要素求交得到交集
    let geoIntersection = await GeometryOperator.intersection(
      this.geometryA,
      this.geometryB,
      this.tolerance,
    );
  ...
  calUnionBtn = async () => {
    ...
    // 两个区几何要素求并得到并集
    let geoUnion = await GeometryOperator.union(
      this.geometryA,
      this.geometryB,
      this.tolerance,
    );
  ...
  calDifferenceBtn = async () => {
    ...
    // 两个几何对象求差值
    let geoDifference = await GeometryOperator.difference(
      this.geometryA,
      this.geometryB,
      this.tolerance,
    );
  ...
  symmetricDifferenceBtn = async () => {
    ...
    // 两个几何对象求对称差
    let geoSymmetricDifference = await GeometryOperator.symDifference(
      this.geometryA,
      this.geometryB,
      this.tolerance,
    );
  ...
  setSplitBtn = async () => {
    ...
    // 两个几何对象求对称差
    let geoSplit = [];
    await GeometryOperator.splitPolygonByLine(
      this.geometryB,
      this.geometryA,
      this.tolerance,
      geoSplit,
    ...
    for (let i = 0; i < geoSplit.length; i++) {
      //将分割结果转为自定义图形
      let graphics = await Graphic.toGraphicsFromGeometry(geoSplit[i]);
      if (graphics.length === 0) {
        ToastAndroid.show('分割失败', ToastAndroid.SHORT);
      } else {
        //将分割结果图形添加到叠加分析结果层
        for (let j = 0; j < graphics.length; j++) {
          let graphicPylygonResObj = await GraphicPolygon.createInstance();
          graphicPylygonResObj = graphics[j];
          if (i === 0) {
  ...
  setMergeBtn = async () => {
    ...
    // 合并两个几何对象
    let geoMerge = await GeometryOperator.dissolve(
      this.geometryA,
      this.geometryB,
      this.tolerance,
    );
