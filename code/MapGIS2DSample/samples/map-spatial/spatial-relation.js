...
export default class MapSpatialRelation extends Component {
    ...
    this.mapLoadListener = DeviceEventEmitter.addListener(
      ...
      async res => {
        if (res.DidFinishLoadingMap) {
          //设置地图初始中心点与缩放范围
          let rectObj = await this.mapView.getDispRange();
  ...
  async initMapView() {
    ...
    let dotObj11 = await Dot.createInstance(12685623, 3594578);
    let dotObj12 = await Dot.createInstance(12685623, 3572194);
    //多边形A
    let dotArrA = [];
    ...
    await this.graphicPolygonAObj.setPoints(dotArrA);
    ...
    await this.graphicPolygonBObj.setPoints(dotArrB);
    ...
    dotArrC.push(dotObj4);
    this.graphicPolygonCObj = await GraphicPolygon.createInstance();
    await this.graphicPolygonCObj.setPoints(dotArrC);
    ...
    dotArrD.push(dotObj1);
    this.graphicPolygonDObj = await GraphicPolygon.createInstance();
    await this.graphicPolygonDObj.setPoints(dotArrD);
    ...
    let graphicTextObj1 = await GraphicText.createInstance();
    let dotObj13 = await Dot.createInstance(12733109.43, 3585761.96);
    ...
    let graphicTextObj2 = await GraphicText.createInstance();
    let dotObj14 = await Dot.createInstance(12739914.3, 3575641.3);
    ...
    let graphicTextObj3 = await GraphicText.createInstance();
    ...
    let graphicTextObj4 = await GraphicText.createInstance();
  ...
  relation = async () => {
    ...
    relationRes = await GeometryOperator.contains(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.crosses(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.disjoint(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.equals(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.overlaps(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.touches(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
    ...
    relationRes = await GeometryOperator.within(
      this.curGeometry1,
      this.curGeometry2,
      this.tolerance,
    );
