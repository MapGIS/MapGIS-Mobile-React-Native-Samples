...
export default class RangeThemeDemo extends Component {
  ...
  initFieldNameData = async itemValue => {
    ...
    let selectedLayer = new VectorLayer();
    selectedLayer.ObjId = mapLayer.ObjId;
  ...
  createRangeTheme = async () => {
    ...
    await queryDef.setFilter("");
    await queryDef.setPagination(1, 20);
  ...
  startCreateTheme = async fieldValue => {
    ...
    // 创建范围专题图对象
    let rangeTheme = await RangeTheme.createInstance();
    ...
    // 创建分段专题图绘制信息对象
    let rangeThemeInfo = await RangeThemeInfo.createInstance();
