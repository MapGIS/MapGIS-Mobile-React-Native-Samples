...
//   openMap = async () => {
//     //设置系统库路径
//     let mapGisEnv = await EnvConfig.getGisEnv();
//     await mapGisEnv.setSlib(SYSTEM_LIB_PATH2);
//     //切换系统库之后必须强制刷新
//     await EnvConfig.setGisEnv(mapGisEnv);
//     let systemLibrarys = await SystemLibrarys.getSystemLibrarys();
//     await systemLibrarys.reOpen();
...
//     let witchLayer = new VectorLayer();
//     witchLayer.ObjId = selectedLayer.ObjId;
...
//     await queryDef.setFilter(''); //设置属性查询条件，为""，则查询到所有要素
//     await queryDef.setPagination(1, 100);  //设置每页结果数目,缺省为10条

//     await queryDef.setSubFields('*');
//     await queryDef.setWithSpatial(false);
...
//         fieldValue.push(featureAttribute[this.state.selectedFieldNameValue]);
//       }
//     }
...
//     // 创建单值专题图
//     let uniqueTheme = await UniqueTheme.createInstance();
...
//     // 创建单值专题图绘制信息对象
//     let uniqueThemeInfo = await UniqueThemeInfo.createInstance();
