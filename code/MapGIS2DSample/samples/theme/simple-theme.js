...
export default class SimpleThemeDemo extends Component {
  ...
  createSimpleTheme = async () => {
    ...
    let themeInfo = await ThemeInfo.createInstance();
    let mapIndex = await this.map.indexOfByName(this.state.selectedValue);
    let mapLayer = await this.map.getLayer(mapIndex);

    let selectLayer = new VectorLayer();
    selectLayer.ObjId = mapLayer.ObjId;
    ...
    // //创建简单专题图对象
    // let simpleTheme = await SimpleTheme.createInstance();
    // //设置专题图的名称
